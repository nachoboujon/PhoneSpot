...
// The goal is to completely replace the catalog logic in script.js to support robust filtering.
// Let's find the entire block that handles 'full-catalog-container'.

const startMatch = "const fullCatalogContainer = document.getElementById('full-catalog-container');";
const endMatch = "// Lógica para Barra de Búsqueda";

if (s.includes(startMatch) && s.includes(endMatch)) {
    const startIndex = s.indexOf(startMatch);
    const endIndex = s.indexOf(endMatch);
    ...
    if (fullCatalogContainer) {
        ...
        const renderFilteredCatalog = () => {
            ...
            let filtered = allCatalogProducts.filter(p => {
                ...
                // Americanos Filter (If selected, only show items with 'americano' or 'usa' in name/desc)
                if (onlyAmericanos) {
                    const str = (p.name + " " + (p.description||'')).toLowerCase();
                    if (!str.includes('americano') && !str.includes('usa') && !str.includes('libre de fabrica')) return false;
                }
            ...
            filtered.forEach(prod => {
                ...
                const cardHTML = \`
                    <div class="product-card" data-id="\${prod.id}" style="position:relative; display:flex; flex-direction:column; background: var(--card-bg); border-radius: 12px; padding: 1.5rem; text-align: center; border: 1px solid var(--border-color); box-shadow: 0 5px 15px rgba(0,0,0,0.05); transition: 0.3s;">
                        \${hasOffer ? \`<span class="badge" style="position:absolute; top:10px; left:10px; background:#ff4757; color:white; padding:4px 8px; border-radius:12px; font-weight:bold; font-size:0.8rem;">-\${discount}%</span>\` : ''}
                        
                        <!-- Favorites Icon -->
                        \${renderFavIconIfEnabled(prod.id)}
        ...
        const renderFavIconIfEnabled = (id) => {
            const favs = JSON.parse(localStorage.getItem('phoneSpotFavs') || '[]');
            const isActive = favs.includes(id.toString()) ? 'active' : '';
            return \`<button class="fav-btn \${isActive}" data-id="\${id}" onclick="toggleFavorite('\${id}', event)" style="position:absolute; top:10px; right:10px; background:rgba(255,255,255,0.9); border:none; width:35px; height:35px; border-radius:50%; box-shadow:0 2px 5px rgba(0,0,0,0.1); cursor:pointer; color: \${isActive ? '#ff4757' : '#ccc'}; transition: 0.3s; z-index:10;"><i class="fa-solid fa-heart"></i></button>\`;
        };
    ...
    s = s.substring(0, startIndex) + newLogic + s.substring(endIndex);
    fs.writeFileSync('public/script.js', s, 'utf8');
    console.log('Successfully rewrote catalog logic.');
} else {
